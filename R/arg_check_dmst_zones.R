...
#'
#' Check the arguments of the dmst.zones, dc.zones, and mlink.zones functions.
#' @return NULL
#' @export
#' @keywords internal
arg_check_dmst_zones = function(coords, cases, pop, w, ex, ubpop, ubd, longlat, type, progress = FALSE) {
  ...
  if (!is.numeric(cases)) stop('cases must be numeric')
  ...
  if (length(progress) != 1 | !is.logical(progress)) stop("progress must be a logical value")
}